...
public class PageRankDriver {
	...
	public static final String MAKER = "adj+";
	public static final double MAX_DIFF = 30;
	
	public static void main(String[] args) throws Exception {
		long startTime = System.currentTimeMillis();
		//String log4jConfPath = "/home/parallels/eclipse-workspace/PageRank.toDist/resources/log4j.properties";
										...
										// data to composite
		{
			...
			} else // In case the function name doesn't match up
			{
				System.err
					...
					// first input is == "compositeWithNames"; parses
					// data to compositeWithNames
		{
		
			if (job.equals("compositeWithNames")) {
				compositeWithNames(args[1], args[2], args[3], args[4], args[5],args[6],
	...
	static void diff(String input1, String input2, String output, int reducers)
		...
		if (job.waitForCompletion(true)) // Once the first job completes
		{
			...
			System.out.println("Diff Part 1 Failed");
		}
	...
	public static void composite(String input, String output, String interim1,
		 ...
		 * TODO 
		 */
		System.out.println("Xiaohan Chang (10424092)");
		...
		while (difference >= PageRankDriver.MAX_DIFF) // continues operation till difference is of specified value
		{
			if (i % 2 == 0) {
	...
	static void names(String input, String output, String names, int reducers)
			throws Exception {
		System.out.println("Names Job Started");
		Path finpath = new Path(names); // Creates new Path
		Configuration conf = new Configuration();
		FileSystem fs = FileSystem.get(URI.create(names), conf);
		...
		job.setMapperClass(FNameMapper.class); // Sets Mapper and Reducer Classes
		job.setReducerClass(FNameReducer.class);
		...
		// Prints message on successful completion or error.
		System.out.print(job.waitForCompletion(true) ? "Names Job Completed" : "Names Job Error");
		// Exits once job finishes.
	}
	
	public static void compositeWithNames(String input, String output, String interim1,
			String interim2, String diff, String namesFolder, int reducers) throws Exception {
			...
			// is the input directory
		{
			...
			counter++;
			finish(interim2, output, reducers);
			names(output, "outputWithNames", namesFolder, reducers);
			summarizeResult("outputWithNames");
		} else // for even i, interim1 is the input directory
		{
			deleteDirectory(interim2); // Deletes other directory
			counter++;
	...
	static void deleteDirectory(String path) throws Exception {
		...
		if (fs.exists(todelete)) {
			...
			System.out.println("no such file to felete:" + path);
		}
